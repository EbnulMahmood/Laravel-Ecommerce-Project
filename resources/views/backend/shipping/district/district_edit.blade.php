        ...
        <div class="row">
            ...
            <div class="col-12 col-md-6 order-md-2 order-first">
                <nav aria-label="breadcrumb" class="breadcrumb-header float-start float-lg-end">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="{{ route('manage.district') }}">All District</a></li>
                            ...
                            <form action="{{ route('district.update', $district->id) }}" method="POST">
                                ...
                                <div class="row">
                                        ...
                                        <select class="choices form-select" required name="division_id">
                                            @foreach ($divisions as $division)
                                                <option value="{{ $division->id }}"
                                                    {{ $division->id == $district->division_id ? 'selected' : '' }}>{{ $division->division_name }}</option>
                                            @endforeach
                                        </select>
                                        @error('division_id')
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input name="district_name" type="text" class="form-control" value="{{ $district->district_name }}" id="basicInput1" placeholder="Enter Name">
                                        @error('district_name')
                                            <span class="text-danger">{{ $message }}</span>    
                                        @enderror
                                    </div>
